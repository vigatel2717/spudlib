...
void spudgpu_cmd_copy_image_to_buffer(
    ...
    VkBufferImageCopy region = spudgpuvulkan___make_buffer_image_copy(
        spud_format_aspect(src_image->_format_vk), desc);

    vkCmdCopyImageToBuffer(
        cmd->_command_buffer_vk,
        src_image->_image_vk,
        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        dst_buffer->_buffer_vk,
        1, &region);
}
...
void spudgpu_cmd_copy_buffer_to_image(
    ...
    VkBufferImageCopy region = spudgpuvulkan___make_buffer_image_copy(
        spud_format_aspect(dst_image->_format_vk), desc);

    vkCmdCopyBufferToImage(
        cmd->_command_buffer_vk,
        src_buffer->_buffer_vk,
        dst_image->_image_vk,
...
void spudgpu_cmd_blit_image(
    ...
    vkCmdBlitImage(
        cmd->_command_buffer_vk,
        src_image->_image_vk, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        dst_image->_image_vk, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        1, &region,
        spud_filter_to_vk(desc->filter));
}
